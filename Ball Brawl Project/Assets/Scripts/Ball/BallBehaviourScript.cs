...
public class BallBehaviourScript : NetworkBehaviour {
    ...
    [SerializeField]
    private GameObject _floorImpact;
    ...
    public void OnCollisionEnter(Collision collision) {
        if (collision.gameObject.tag == Tags.WALL) {
            ...
            if (isServer) {
                GameObject floorImpact = Instantiate(_floorImpact, collision.contacts[0].point, Quaternion.LookRotation(collision.contacts[0].normal, transform.up));
                NetworkServer.Spawn(floorImpact);
                Destroy(floorImpact, 0.5f);
            }
        }
    }
